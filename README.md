...
## Introduction
 The goal of this project is to familiarize with ROS and Create the map in a certain environment and autonomously move the Robot (Turtle bot 2) to the desired location in the map where we have to scan the QR code and the detect the text. 
...
```
5) launch .py file for Qr code detection(workstation)
```
roslaunch rbx1_vision cv_hardik.launch
```
6) rviz launch (workstation)
 ```
 ...
 ```
 8) launch file for goal 1 position to goal 2 position (Turtlebot)
 ```
